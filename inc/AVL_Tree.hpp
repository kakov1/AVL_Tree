...
protected:
  ...
  public:
    ...
    bool operator!=(const SearchTreeIt &rhs) const { return !(*this == rhs); }

    const KeyT &operator*() const { return current_->key_; }
  };

  std::size_t get_height(Node *node) const { return node ? node->height_ : 0; }
...
public:
  ...
  SearchTreeIt lower_bound(const KeyT &key) const {
    ...
    if (comp_(max_node->key_, cur_node->key_) && comp_(cur_node->key_, key)) {
      max_node = cur_node;
    }
  ...
  SearchTreeIt upper_bound(const KeyT &key) const {
    ...
    if (comp_(cur_node->key_, min_node->key_) && comp_(key, cur_node->key_)) {
      min_node = cur_node;
    }
